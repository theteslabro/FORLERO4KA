...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function setNavArrowVisibility(toMonth) {
        if (navLeft) navLeft.classList.toggle('nav-hidden', toMonth === 1);
        if (navRight) navRight.classList.toggle('nav-hidden', toMonth === totalMonths);
    }
    ...
    if (navRight) navRight.addEventListener('click', () => {
        if (currentMonth < totalMonths) switchMonth(currentMonth + 1);
    });
    if (navLeft) navLeft.addEventListener('click', () => {
        if (currentMonth > 1) switchMonth(currentMonth - 1);
    });
    ...
    function initMonth5() {
        ...
        const songs = [
            { title: '♪ Наша песня', desc: 'Та самая, от которой мурашки каждый раз', color: '#ff7da3' },
            { title: '♪ Romantic Vibes', desc: 'Под эту музыку я думаю о тебе', color: '#c084fc' },
            { title: '♪ Ночной Drive', desc: 'Для наших долгих разговоров до рассвета', color: '#60a5fa' },
            { title: '♪ Обнимашки FM', desc: 'Когда хочется тепла и уюта', color: '#34d399' },
            { title: '♪ Танец вдвоём', desc: 'Представь, мы танцуем медленный танец...', color: '#fbbf24' },
            { title: '♪ Вечная история', desc: 'Наша любовь — это целый саундтрек', color: '#f43f5e' }
        ];
        ...
        if (rouletteWheel) {
            songs.forEach((song, i) => {
                ...
                const angle = (360 / songs.length) * i;
                segment.style.cssText = `--seg-color: ${song.color}; transform: rotate(${angle}deg);`;
                segment.innerHTML = `<span class="seg-label">${song.title.substring(2, 10)}...</span>`;
        ...
        if (rouletteSpin) {
            rouletteSpin.addEventListener('click', () => {
                ...
                rouletteWheel.style.transition = 'transform 3s cubic-bezier(0.15, 0.75, 0.12, 0.99)';
