...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function switchMonth(toMonth) {
        ...
        setTimeout(() => {
            // Очищаем классы active у всех
            month1View.classList.remove('active', 'exit-left', 'exit-right');
            month2View.classList.remove('active', 'exit-left', 'exit-right');
            if (month3View) month3View.classList.remove('active', 'exit-left', 'exit-right');

            // Устанавливаем exit-классы для предыдущего активного месяца
            if (currentMonth < toMonth) {
                if (currentMonth === 1) month1View.classList.add('exit-left');
                if (currentMonth === 2) month2View.classList.add('exit-left');
            } else {
                if (currentMonth === 2) month2View.classList.add('exit-right');
                if (currentMonth === 3 && month3View) month3View.classList.add('exit-right');
            }

            // Показываем новый активный месяц
            if (toMonth === 1) {
                ...
                if (month3View) month3View.classList.add('active');
                navLeft.style.display = 'flex';
    ...
    if (navRight) navRight.addEventListener('click', () => {
        if (currentMonth < 3) switchMonth(currentMonth + 1);
    });
